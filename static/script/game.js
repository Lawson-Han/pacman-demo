...
function Game(id,params){
    ...
    var Item = function(params){
        ...
        this._settings = {
            x:0,					// Position coordinate: x-axis
            y:0,					// Position coordinate: y-axis
            width:20,				// Width
            height:20,				// Height
            type:0,					// Object type, 0=normal object (not map bound), 1=player controlled, 2=program controlled
            color:'#F00',			// Identification color
            status:1,				// Object status, 0=inactive/ended, 1=normal, 2=paused, 3=temporary, 4=abnormal
            orientation:0,			// Current orientation, 0=right, 1=down, 2=left, 3=up
            speed:0,				// Movement speed
            // Map related
            location:null,			// Position map, Map object
            coord:null,				// Map coordinates if object is bound to map; position coordinates if not bound
            path:[],				// NPC automatic walking path
            vector:null,			// Target coordinate
            // Layout related
            frames:1,				// Speed level, internal counter for how many frames times changes
            times:0,				// Canvas refresh counter (for cyclic animation state judgment)
            timeout:0,				// Countdown (for process animation state judgment)
            control:{},				// Control cache, processed when reaching positioning point
    ...
    var Map = function(params){
        ...
        this._stage = null;         // Bind to associated stage
        this._settings = {
            x:0,					// Map starting coordinate
            y:0,
            size:20,				// Map unit width
            data:[],				// Map data
            x_length:0,				// 2D array x-axis length
            y_length:0,				// 2D array y-axis length
            frames:1,				// Speed level, internal counter for how many frames times changes
            times:0,				// Canvas refresh counter (for cyclic animation state judgment)
            cache:false,    		// Whether static (set cache if static)
            update:function(){},	// Update map data
            draw:function(){},		// Draw map
        };
        Object.assign(this,this._settings,this._params);
    };
    // Get value of a point on the map
    ...
    // Set value of a point on the map
    Map.prototype.set = function(x,y,value){
        if(this.data[y]){
            this.data[y][x] = value;
        }
    };
    // Map coordinate to canvas coordinate
    ...
    var Stage = function(params){
        ...
        this._settings = {
            index:0,                        // Stage index
            status:0,						// Stage status, 0=inactive/ended, 1=normal, 2=paused, 3=temporary status
            maps:[],						// Map queue
            audio:[],						// Audio resources
            images:[],						// Image resources
            items:[],						// Object queue
            timeout:0,						// Countdown (for process animation state judgment)
            update:function(){}				// Sniff, handle relative relationships of different objects in layout
        };
        Object.assign(this,this._settings,this._params);
    };
    // Add object
    ...
    this.start = function() {
        ...
        var fn = function(){
            ...
            if(stage.update()!=false){		            // If update returns false, do not draw
                ...
                stage.items.forEach(function(item){
                    ...
                    if(stage.status==1&&item.status!=2){  	// Object and stage status are not in paused state
                        if(item.location){
